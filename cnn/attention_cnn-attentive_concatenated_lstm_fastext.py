...
model.add(Reshape((max_news_length,max_sent_length,300)))
#model.add(TimeDistributed(Conv1D(filters=required_sent_embedding_size,kernel_size=3)))
model.add(TimeDistributed(Bidirectional(LSTM(lstm_num_hidden,return_sequences=True))))
model.add(TimeDistributed(Bidirectional(LSTM(lstm_num_hidden,return_sequences=True))))
model.add(TimeDistributed(Bidirectional(LSTM(lstm_num_hidden))))
#model.add(Reshape((max_news_length,98,required_sent_embedding_size)))
#model.add(TimeDistributed(AttentionLayer(hidden_layers=(75,))))
...
for i in range(20):
	...
	if val_f1>max_f1:
		max_f1=val_f1
		model.save('bestmodels/dblstm_dac_dblstm_da_fst_trem_f1')
		print 'Saved model'
	else:
		print ''
