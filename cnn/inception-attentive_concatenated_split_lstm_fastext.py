...
dac=Reshape((4,25,2*required_sent_embedding_size))(dac)
dbgru=TimeDistributed(Bidirectional(GRU(lstm_num_hidden/2,return_sequences=True)))(dac)
dbgru=TimeDistributed(Bidirectional(GRU(lstm_num_hidden/2,return_sequences=True)))(dbgru)
dbgru=TimeDistributed(Bidirectional(GRU(lstm_num_hidden/2,return_sequences=True)))(dbgru)
da=TimeDistributed(AttentionLayer(hidden_layers=(75,)))(dbgru)
da=Reshape((4*lstm_num_hidden,))(da)
FC=Dropout(0.3)(da)
...
for i in range(20):
	...
	if val_f1>max_f1:
		max_f1=val_f1
		model.save('bestmodels/ca-inception_conv_dac_sdbgru_da_te_trem_f1')
		print 'Saved model'
	else:
		print ''
