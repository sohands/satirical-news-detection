...
save_filename='models/cnn_bidirectional_lstm_transformed_embeddings_1'
...
model.add(TimeDistributed(Flatten()))
model.add(Bidirectional(LSTM(lstm_num_hidden)))
model.add(Dropout(0.3))
model.add(Dense(100,activation='relu'))
model.add(Dropout(0.3))
model.add(Dense(2,activation='softmax'))
...
checkpoint=ModelCheckpoint('bestmodels/cnn_bidirectional_lstm__transformed_embeddings',monitor='val_acc',save_best_only=True,mode='max',verbose=1)
