...
save_filename='models/cnn_ac_blstm_a_te_1'
...
model.add(TimeDistributed(AttentiveConcatenateLayer(length=max_sent_length)))
...
model.add(TimeDistributed(Flatten()))
model.add(AttentiveConcatenateLayer(length=max_news_length))
...
checkpoint=ModelCheckpoint('bestmodels/cnn_ac_blstm_a_te',monitor='val_acc',save_best_only=True,mode='max',verbose=1)

model.compile(optimizer=optimizer,loss=loss,metrics=metrics)

model.fit(trainX,trainY,epochs=20,batch_size=3,validation_data=valdata,callbacks=[checkpoint])
