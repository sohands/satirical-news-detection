...
#valdata=(valX,valY)
#testX,testY=loadTrainData(DATA_FOLDER+'train_data_test_X.npz',DATA_FOLDER+'train_data_test_Y.npz')
...
syntax_X_test=np.load('syntax_X_test.npz')['arr_0']
syntax_info_length=105 	
#valX=[valX,syntax_X_val]
#testX=[testX,syntax_X_test]
...
model.add(TimeDistributed(Flatten()))
#model.add(Dropout(0.3))
model.add(AttentiveConcatenateLayer(length=max_news_length,hidden_layers=()))
model.add(Bidirectional(GRU(lstm_num_hidden,return_sequences=True)))
model.add(Bidirectional(GRU(lstm_num_hidden,return_sequences=True)))
#model.add(Bidirectional(GRU(lstm_num_hidden,return_sequences=True)))
model.add(AttentionLayer(hidden_layers=()))
model.add(Dropout(0.3))
model.add(Dense(100,activation='relu'))
model.add(Dropout(0.3))
model.add(Dense(2,activation='softmax'))
...
for j in range(20):
	for i in range(0,len(trainX),10000):
		...
		model.fit([trainX[i:min(len(trainX),i+10000)],syntax_X],trainY[i:min(len(trainX),i+10000)],epochs=1,batch_size=24)
	# model.save('epochmodels/cnn_dac_dbgru_da_te_'+str(i))
	#val_f1=F1(valY,model.predict(valX,batch_size=24))
	#print 'Epoch',j,': Validation F1 score:',val_f1,
	print 'Epoch',j
        print 'Validation'
	valX,valY=loadTrainData(DATA_FOLDER+'train_data_dev_X.npz',DATA_FOLDER+'train_data_dev_Y.npz')
        print_result(valY,model.predict([valX,syntax_X_val]))
        print 'Test'
	valX,valY=loadTrainData(DATA_FOLDER+'train_data_test_X.npz',DATA_FOLDER+'train_data_test_Y.npz')
        print_result(valY,model.predict([valX,syntax_X_test]))
        print ''
	continue
	if val_f1>max_f1:
		max_f1=val_f1
		model.save('bestmodels/cnn_dac_dbgru_dab_te_syntax_oh_trem_f1')
		print 'Saved model'
	else:
		print ''
