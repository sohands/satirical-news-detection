...
doc_syntax_dev=np.load('doc_syntax_dev.npz')['arr_0']/100000
...
doc_syntax_input=Input(shape=(105,))

da=concatenate([da,doc_syntax_input])
...
model=Model(inputs=[original_inputs,syntax_input,doc_syntax_input],outputs=predictions)
...
for j in range(20):
        for i in range(0,len(trainX),10000):
                ...
                doc_syntax_X=np.load('./doc_syntax/syntax_X_'+str(min(len(trainX),i+10000))+'.npz')['arr_0']/100000
                model.fit([trainX[i:min(len(trainX),i+10000)],syntax_X,doc_syntax_X],trainY[i:min(len(trainX),i+10000)],epochs=1,batch_size=24)
        ...
        if val_f1>max_f1:
                max_f1=val_f1
                model.save('bestmodels/a-inception_dac_bgru_da_te_syntax_oh_trem_ds_f1')
                print 'Saved model'
        else:
                print ''
