...
conv1x1=TimeDistributed(Conv1D(filters=required_sent_embedding_size/2,kernel_size=1,activation='relu'))(embeddings)
...
conv3x1=TimeDistributed(Conv1D(filters=required_sent_embedding_size/2,kernel_size=1))(embeddings)
...
conv5x1=TimeDistributed(Conv1D(filters=required_sent_embedding_size/2,kernel_size=1))(embeddings)
...
dbgru=Bidirectional(LSTM(lstm_num_hidden,return_sequences=True))(dac)
dbgru=Bidirectional(LSTM(lstm_num_hidden,return_sequences=True))(dbgru)
dbgru=Bidirectional(LSTM(lstm_num_hidden,return_sequences=True))(dbgru)
...
for j in range(20):
        ...
        if val_f1>max_f1:
                max_f1=val_f1
                model.save('bestmodels/a-inception_dac_dblstm_da_te_syntax_oh_trem__reg_f1')
                print 'Saved model'
        else:
                print ''
